...
export const createOrderAction = cache(async (encodedData: string) => {
  // Destructure
  const { cart, customer, country, country_iso_3, initialCurrency, delivery_address } = JSON.parse(
    decrypt(encodedData),
  ) as CreateOrderActionInterface;

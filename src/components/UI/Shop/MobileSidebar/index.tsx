...
const ShopMobileSideBar: FC<SideDrawerInterface> = ({ openState, openCloseController }) => {
  ...
  return (
    <Drawer direction='left' open={openState} onOpenChange={openCloseController}>
      <DrawerOverlay className='bg-black/[0.01] !backdrop-blur-[2px] lg:hidden' />
      <DrawerContent
        id='drawer-root'
        className='!rounded-none min-h-[100svh] h-[100svh] bg-[#131313]/95 !border-none
              !fixed !bottom-0 !left-0 !z-[500] w-full max-w-[350px] overflow-y-auto overscroll-contain pb-[env(safe-area-inset-bottom)] lg:hidden'
      >

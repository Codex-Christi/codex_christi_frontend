...
export default function AdminTopBar({ title, subtitle, onOpenMobileNav }: AdminTopBarProps) {
  return (
    <header className='sticky top-0 z-40 flex min-h-20 flex-wrap items-center gap-3 border-b border-white/10 bg-slate-950/74 px-3 py-3 supports-[backdrop-filter]:backdrop-blur-xl sm:gap-4 sm:px-5'>
      ...
      <div className='order-last flex min-w-full flex-1 items-center gap-2 rounded-lg border border-white/10 bg-slate-900/55 px-3 py-2 text-slate-400 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:order-none sm:min-w-[320px] xl:max-w-[470px]'>
        <Search size={16} />
      ...
      <div className='ml-auto flex items-center gap-3'>
        <span className='hidden items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-slate-100 sm:inline-flex'>
        ...
        <RefreshCw className='hidden text-emerald-300 md:block' size={22} />
        ...
        <div className='hidden h-10 w-10 place-items-center rounded-full bg-white/10 text-sm font-semibold text-white sm:grid'>
          AD
        </div>
      </div>
    </header>

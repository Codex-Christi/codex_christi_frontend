...
export default function AdminDashboardNavigationList({
  ...
  return (
    <nav
      className={cn(
        'flex flex-col gap-4',
        mode === 'desktop' ? 'min-h-0 flex-1 overflow-y-auto px-3 py-3' : 'px-3 pb-4',
      )}
    >
      {groups.map((group) => (
...
function AdminDashboardNavigationItem({
  ...
  const className = cn(
    'inline-flex w-full items-center gap-3 rounded-lg border text-left text-sm font-medium transition',

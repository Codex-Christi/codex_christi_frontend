...
const statusCopy: Record<string, { title: string; description: string }> = {
  processing: {
    ...
    description: 'Please keep this tab open while we finish the final verification steps.',
  },
  completed: {
    title: 'Order locked in',
    description:
      'Everything looks great. We are packaging your order and will notify you via email.',
  },
  error: {
    title: 'Something needs your attention',
    description:
      'One of the processing steps failed. Please retry your payment or contact support if the issue persists.',
  },
  idle: {
    title: 'Order status unavailable',
    description: 'We could not find a recent payment. Please return to the shop to start again.',
  },
};
...
const CheckoutConfirmationPage = ({ params }: PageProps) => {
  const routeOrderId = use(params)?.orderId;
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      steps: state.steps,
      pdfReceiptLink: state.pdfReceiptLink,
      receiptFileName: state.receiptFileName,
    })),
  );

  const currentStatusCopy = statusCopy[flowStatus] ?? statusCopy.idle;
  const orderIdMismatch = routeOrderId && orderId && routeOrderId !== orderId;
  const canDownloadReceipt = Boolean(pdfReceiptLink && receiptFileName);
  ...
  const handleDownLoad = useCallback(async () => {
    try {
      if (!canDownloadReceipt || !pdfReceiptLink || !receiptFileName) {
        throw new Error('Receipt is not ready yet.');
      }
      setDownloading(true);
      const directDownload = () => {
        const link = document.createElement('a');
        link.href = pdfReceiptLink;
      ...
      try {
        const receiptUrl = new URL(pdfReceiptLink, window.location.href);
  ...
  }, [canDownloadReceipt, pdfReceiptLink, receiptFileName]);
  ...
  }, [clearCart, clearVerifiedEmails, resetCheckoutToInitial, setOrderString]);
  ...
  if (!orderId || orderIdMismatch) {
    return (
      <div className='flex min-h-[60vh] flex-col items-center justify-center gap-6 px-4 text-center text-white'>
        <h1 className='text-2xl font-semibold tracking-tight'>We could not find that order</h1>
        <p className='text-white/70 max-w-lg'>
          If you just completed a payment, please keep this page open so we can finish processing
          and try again in a few seconds. Otherwise return to the shop and start a fresh checkout.
        </p>
        <CustomShopLink
          href='/shop'
    ...
    <div className='!text-white flex flex-col py-4 !font-inter !select-none'>
      ...
      >
        ...
        <div className='flex flex-col items-center text-center space-y-4'>
          ...
          <div>
            <p className='text-sm uppercase tracking-[0.2em] text-white/60'>Order #{orderId}</p>
        ...
        <div className='w-full space-y-3'>
          ...
          {/* <CustomShopLink
            ...
            className='block text-center py-3 rounded-2xl border border-[#2576CE] bg-[#2576CE] text-[#F3F3F3] font-semibold'
          >
            View order history
          </CustomShopLink> */}
        </div>

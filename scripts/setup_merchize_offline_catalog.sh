...
echo "→ Running setup_merchize_offline_catalog.sh from project root: $ROOT_DIR"

run_yarn_install_for_current_major() {
  local yarn_version
  local yarn_major
...
if [ "$FORCE_PUSH" -eq 1 ]; then
  ...
  MERCHIZE_OFFLINE_CATALOG_SCHEMA_CHANGED=1
  ...
  MERCHIZE_OFFLINE_CATALOG_SCHEMA_CHANGED=0
fi
...
MERCHIZE_OFFLINE_CATALOG_DATABASE_URL="file:./data/db/shop/merchizeCatalog.db"
EOF
else
...
CATALOG_DB_URL="${MERCHIZE_OFFLINE_CATALOG_DATABASE_URL:-${MERCHIZE_PRICE_CATALOG_DATABASE_URL:-file:${ROOT_DIR}/data/db/shop/merchizeCatalog.db}}"
export MERCHIZE_OFFLINE_CATALOG_DATABASE_URL="$CATALOG_DB_URL"
export MERCHIZE_PRICE_CATALOG_DATABASE_URL="$CATALOG_DB_URL"
echo "→ Using offline catalog DB URL: $MERCHIZE_OFFLINE_CATALOG_DATABASE_URL"
...
0 3 * * * cd /root/apps/codexchristi && /usr/bin/env bash scripts/setup_merchize_offline_catalog.sh --no-deps --no-build >> /var/log/merchize_offline_catalog_setup.log 2>&1

EOF
fi

echo "✅ Merchize offline catalog setup complete."
